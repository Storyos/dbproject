...
async function searchbook(connection) {
  ...
  let sql = `select book_title 책_제목, book_author 저자, publisher 출판사, DATE_FORMAT(publishing_year, '%Y-%m-%d') 출간일, book_state 현재도서상태 from book where book_title like ? or book_author like ? `;
  ...
  console.log("1. 돌아가기 2. 종료");
  let book_cmd = await Input.getUserInput();
  if(book_cmd==='2'){
    connection.end();
    process.exit();
  }
